...
public:
    // Zagnieżdżona klasa Iterator
    class Iter:public Iterator<int> 
    {
        std::vector<bool> data;
        int universeSize;
        int index;
        friend class SetAsArray;
    public:
        Iter(const std::vector<bool>& arr, int us);
        ~Iter();
...
SetAsArray::Iter::Iter(const std::vector<bool>& array, int us) {
    data = array;
    universeSize = us;

    int i = 0;
    while (i < data.size()) { //Konstruktor ustawia index na pierwszą komórke z wartością 
        if (data[i]) {
            index = i;
            return;
        }
    ...
    index = universeSize; //Jeśli żadna nie ma to universeSize
}

const int& SetAsArray::Iter::operator*() const {
    return index; //Zwraca index
}

void SetAsArray::Iter::operator++() {
    int i = index + 1;
    while (i < data.size()) { //Zaczynamy od komórki większej od index i idziemy do góry
        if (data[i]) { //Jeśli któraś ma wartość to przypisujemy ją index
            index = i; 
            return;
        }
        i++;
    }
    index = universeSize; //Jeśli nie to ustawiamy index jako universesize dla funkcji IsDone()
}

bool SetAsArray::Iter::IsDone() const {
    return index >= universeSize; //Jeśli index jest większy bądź równy od universesize czyli jesteśmy poza tablicą to zwraca true
}

SetAsArray::Iter::~Iter() {
    // Destruktor żeby kompilator nie miał problemów
}
...
int main () {
    ...
    SetAsArray::Iter& it_A = A.NewIterator();
    std::cout << "Elementy A używając iteratora" << std::endl;
    while (!it_A.IsDone()) {
        int element = *it_A;
        std::cout << element << " ";
        ++it_A;
    ...
    std::cout << std::endl;
    delete &it_A; // Do zapobiegania wyciekom

    std::cout << std::endl;

    SetAsArray::Iter& it_B = B.NewIterator();
    std::cout << "Elementy B używając iteratora" << std::endl;
    while (!it_B.IsDone()) {
        int element = *it_B;
        std::cout << element << " ";
        ++it_B;
